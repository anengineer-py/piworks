...
I took the inputs as a 2D matrix.
Then in coding, it finds the largest number of the first line and then checks whether that number is prime or not.
It then checks the straight bottom and diagonal line of the specified number if the number is not a prime number.
After finding the big one, it goes to the bottom regularly.

Finally, it collects the big ones in all lines and writes it as a result.

* If the two numbers it controls are both prime numbers, it accepts those lines as '0' and continues the operations under the largest one.